...
<h3 class="text-center my-5">Quiz results for your query</h3>
...
//    $quizzes = $QAcc->getNumQuestionsOfAllQuizzes();
        $quizzes = $QAcc->getAllQuizzes();

        foreach ($quizzes as $quiz)
        {
            echo "<tr>
                    <th scope='row'>{$quiz->getQuizID()}</th>
                    <td>{$quiz->getQuizTitle()}</td>
                    <td>".count($quiz->getQuestions())."</td>
                    <td>
                        <form>
                            <input type='hidden' name='quizID' value='{$quiz->getQuizID()}'>
                            <button type='submit' class='btn btn-outline-success'>Take quiz</button>
                        </form>
                    </td>
                </tr>";
        }
