...
<form action="../quizResults.html" >
    <?php
    $ra = new QuizResultAccessor();
    $qa = new QuizAccessor();
    $quizID = $_GET["quizID"];
    $resultID = $_GET["quizResultID"];
    //ChromePhp::log($quizID);
    $tempQuiz = $qa->getQuizByID($quizID);
    $tempResult = $ra->getResultsByID($resultID);
    $quiz =$tempQuiz->jsonSerialize();
    $result = $tempResult[0]->jsonSerialize();
    ChromePhp::log($result);
    ChromePhp::log($tempQuiz);
    $length = count($quiz["questions"]);

    $html = "";
    $counter = 0;
    for($i = 0; $i < $length; $i++){
        $question = $quiz["questions"][$i]->jsonSerialize();
